...
export async function generateMetadata({ params }) {
  ...
  // cari produk berdasarkan slug otomatis
  const product = productsData.find((item) => slugify(item.title) === slug);

  // panggil headers() DI DALAM FUNCTION
...
export default function ProductDetail({ params }) {
  ...
  // ambil hostname dari server di dalam komponen (tidak di top-level)
  const headersList = headers();
  const host = headersList.get("host");
  const protocol = host.includes("localhost") ? "http" : "https";
  ...
  // teks share (judul + deskripsi 160 karakter + url)
